...
import type { UserLayer, Annotation, PanoImage, RasterLayerData } from "@/types";
import {
  parseGeoJSON,
  parseKMLOrKMZ,
  parseShapefile,
...
export default function LayersPanel({
  ...
  const userImageryLayers = userLayers.filter(
    (layer) => layer.type === "raster" && !(layer.data as RasterLayerData).dem
  );
  const userElevationLayers = userLayers.filter(
    (layer) => layer.type === "raster" && (layer.data as RasterLayerData).dem
  );
          ...
          case "tiff": {
            ...
            const rasterLayers = await parseGeoTIFF(file);
             if (rasterLayers.length > 0) {
              rasterLayers.forEach(onAddLayer);
               toast({
                title: "Success",
  ...
  const getUserLayerIcon = (layer: UserLayer) => {
    ...
    // Simple check for geometry type if possible, otherwise default vector icon
    if (layer.data.type === "FeatureCollection") {
      const firstFeatureGeom = layer.data.features[0]?.geometry?.type;
