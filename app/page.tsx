...
export default function Home() {
 ...
 const addUserLayer = (
    ...
    // Add detailed logging when layer is added
    ...
    if (layer.type === "raster") {
      ...
      console.log(`=== RASTER DATA DEBUG ===`);
      console.log(`Bands: ${rasterData.bands} (type: ${typeof rasterData.bands})`);
    ...
    console.log(`=== FINAL LAYER OBJECT ===`);
    console.log(`Final layer:`, newLayer);
    console.log(`=========================`);
    ...
    if (data.locationMode === 'metadata') {
      try {
        ...
        if (tags.GPSLatitude && tags.GPSLongitude) { 
          const latValue = parseFloat(tags.GPSLatitude?.description || "");
          const lonValue = parseFloat(tags.GPSLongitude?.description || "");

          const lat = latValue * (tags.GPSLatitudeRef?.description === 'S' ? -1 : 1);
          const lon = lonValue * (tags.GPSLongitudeRef?.description === 'W' ? -1 : 1);
            ...
            <div className="relative h-full w-full">
              ...
              <div className="absolute bottom-4 left-4 z-10 h-1/4 min-h-[150px] w-1/3 min-w-[250px] max-w-[400px] overflow-hidden rounded-md border-2 border-card bg-background shadow-lg">
                <MapComponent
                  ...
                  onPanoMarkerClick={(id: string) => {
                    setSelectedPanoId(id);
                  }}
                />
              </div>
            </div>
